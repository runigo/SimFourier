...
int texturesSelectifsInitial(texturesT * textures, affichageT * affichage)
	...
	(*textures).selectifInitial[15] = SDL_CreateTextureFromSurface((*affichage).rendu, image);
	SDL_FreeSurface(image);
	if ((*textures).selectifInitial[15] == 0)
		{
		fprintf(stderr,"grapheInitialisation : Erreur creation texture : %s\n",SDL_GetError());
		}
...
int texturesSelectifsGraphes(texturesT * textures, affichageT * affichage)
	...
	image = SDL_LoadBMP("./image/axes.bmp");
	if (!image)
		{
		fprintf(stderr,"Erreur chargement image ./image/axes.bmp : %s\n",SDL_GetError());
		}
